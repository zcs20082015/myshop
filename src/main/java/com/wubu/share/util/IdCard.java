...
/**
 * 中国公民身份证校验处理辅助类 1、号码的结构 公民身份号码是特征组合码，由十七位数字本体码和一位校验码组成。排列顺序从左至右依次为：六位数字地址码，
...
public class IdCard {
	...
	private static IdCard _instance;
	...
	public boolean IdCardValidate(String IDStr) {
		log.info("进入身份证号[" + IDStr + "]合法性校验处理方法");
		if (IDStr == null)
			return false;
		...
		if (IDStr.length() != 15 && IDStr.length() != 18) {
			errorInfo = "身份证号码[" + IDStr + "]校验失败,号码长度应该为15位或18位。";
			log.info(errorInfo);
			return false;
		}
		// =======================(end)========================
		...
		if (isNumeric(Ai) == false) {
			errorInfo = "身份证号码[" + IDStr
			        + "]校验失败,15位号码都应为数字;18位号码除最后一位外，都应为数字。";
			log.info(errorInfo);
			return false;
		}
		// ================ 出生年月是否有效 ================
		...
		if (this.isDate(strYear + "-" + strMonth + "-" + strDay) == false) {
			errorInfo = "身份证号码[" + IDStr + "]校验失败,生日无效。";
			log.info(errorInfo);
			return false;
		}
		...
		try {
			if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150
				...
				errorInfo = "身份证号码[" + IDStr + "]校验失败,生日不在有效范围。";
				log.info(errorInfo);
				return false;
			}
		}
		...
		if (Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0) {
			errorInfo = "身份证号码[" + IDStr + "]校验失败,月份无效";
			log.info(errorInfo);
			return false;
		}
		if (Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0) {
			errorInfo = "身份证号码[" + IDStr + "]校验失败,日期无效";
			log.info(errorInfo);
			return false;
		}
		// =====================(end)=====================
		...
		if (h.get(Ai.substring(0, 2)) == null) {
			errorInfo = "身份证号码[" + IDStr + "]校验失败,地区编码错误。";
			log.info(errorInfo);
			return false;
		}
		// ==============================================
		...
		if (IDStr.length() == 18) {
			if (Ai.equalsIgnoreCase(IDStr) == false) {
				errorInfo = "身份证号码[" + IDStr + "]校验失败,身份证无效，最后一位字母错误";
				log.info(errorInfo);
				return false;
			}
		}
		else {
			log.info("身份证号码[" + IDStr + "]校验成功,所在地区:"
